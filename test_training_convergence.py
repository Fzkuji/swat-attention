...
tau_triton = torch.nn.Parameter(torch.full((H,), tau_init, device=device, dtype=torch.bfloat16))
bias_triton = torch.nn.Parameter(torch.randn(H, 512, device=device, dtype=torch.bfloat16) * 0.02)
...
for step in range(num_steps):
    ...
    out = lazy_attention_triton(q, k, v, bias_triton, tau_triton)
