...
echo "freeze_lazy:      ${freeze_lazy:=}"
echo "lazy_delayed:     ${lazy_delayed:=}"
echo "node:             ${node:=}"
echo "rank:             ${rank:=}"
echo "ip:               ${ip:=}"
echo "port:             ${port:=}"
...
if [ "$freeze_lazy" != "" ]; then
  params+=" --freeze_lazy_from_checkpoint $freeze_lazy --lazy_total_steps_multiplier 0"
fi
if [ "$lazy_delayed" != "" ]; then
